...
int main(int argc, char **argv) {
			...
			case 0:
				switch (option_index) {
    				...
    				case 4:
    				    timeout = atoi(optarg);
    				    if(timeout <= 0){
    				        printf("ERROR: timeout is a positive number\n");
    						return 1;
    				    }
    					break;
    
    				defalut:
    					printf("ERROR: Index %d is out of options\n", option_index);
    				}
				    break;
			case 'f':
				with_files = true;
	...
	if (optind < argc) {
		printf("ERROR: Has at least one no option argument\n");
		return 1;
	}
	...
	const char path_to_save[] = "something/with_files";
